...
class JoyBridge():
    ...
    def _circle_traj(self, v, t):
        self.v1x = AMPLITUDE*sin(v*(2*pi)/SIDES)
        self.v1y = AMPLITUDE*cos(v*(2*pi)/SIDES)
        self.v2x = AMPLITUDE*sin((v+1)*(2*pi)/SIDES)
        self.v2y = AMPLITUDE*cos((v+1)*(2*pi)/SIDES)

        self.x = self.v1x + (self.v2x - self.v1x)*t;
        self.y = self.v1y + (self.v2y - self.v1y)*t;
        self._inv_kinematics()

    def run(self):
        timer = rospy.Rate(UPDATE_RATE)
        side_time = 1.0/(FREQ*SIDES)
        time = 0.0
        vertex = 0.0
        while not rospy.is_shutdown():
            self._circle_traj(vertex, time/side_time)
            self._send_setpoint()
            timer.sleep()
